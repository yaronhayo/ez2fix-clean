// Service Areas API endpoint for secure map data
...
const serviceAreas = [
  ...
  { name: 'Carlstadt', slug: 'carlstadt', lat: 40.8390, lng: -74.0893 },
  ...
  { name: 'Montclair', slug: 'montclair', lat: 40.8176, lng: -74.2090 },
...
export const GET: APIRoute = async ({ request, url }) => {
  try {
    ...
    if (mapType === 'service-areas') {
      // Return service area data with coordinates for map markers
      const mapData = {
    ...
    // Static map URL generation (server-side)
    if (mapType === 'static') {
      ...
      // Build markers string for service areas
      const markers = serviceAreas.slice(0, 10).map(area => 
        `color:0xEECD5C|label:${area.name.charAt(0)}|${area.lat},${area.lng}`
      ).join('&markers=');
      
      const staticMapUrl = `https://maps.googleapis.com/maps/api/staticmap?` +
        `key=${serverEnv.googleMaps?.apiKey || ''}&` +
        `center=${center}&` +
        `zoom=${zoom}&` +
        `size=${width}x${height}&` +
        `maptype=roadmap&` +
        `markers=color:0xD2A63C|label:B|${center}&` +
        `markers=${markers}`;
